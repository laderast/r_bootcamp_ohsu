...
# Reminder to open up the `part2.Rproj` file before you do anything
...
Link to video: <https://youtu.be/EJ6_DEhC8v4?t=3270>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=3270")
...
Link to video: <https://youtu.be/EJ6_DEhC8v4?t=3463>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=3463")
```
...
<https://youtu.be/EJ6_DEhC8v4?t=5532>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=5532")
```
...
## Further Reading: Organizing Data in Spreadsheets

One paper I think you should all read is Kara Woo and Karl Broman's paper [Organizing Data in Spreadsheets](https://www.tandfonline.com/doi/full/10.1080/00031305.2017.1375989). This is one of the best papers about how to organize data that I know of.
...
Link to video: <https://youtu.be/EJ6_DEhC8v4?t=6056>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=6056")
```
...
As we learned in our last session, a vector needs to include all of the same type of data (e.g., `character`, `numeric`).
...
We'll see that `View()` can help you out when you're starting out with building data processing with `dplyr`, but that's for next time.

### Summarizing our Data

<https://youtu.be/EJ6_DEhC8v4?t=6528>

```{r}
vembedr::embed_url("https://youtu.be/yCPcijCj15c?t=6528")
...
Now we can use it. There is one main function called `skim()` in `skimr`.

One thing to keep in mind: you only need to install a package with `install.packages()` once. Once it's installed, you can use the `library()` command to load it up.

Here we're loading the `skimr` library and then we're using a function in it called `skim()`:
...
Link to video: <https://youtu.be/EJ6_DEhC8v4?t=7023>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=7023")
```
...
Based on the Age versus Days to Last Followup graph above, map the appropriate variables in `brca_clinical` to the x, and y aesthetics. Run your plot. Is it what you expected?
...
<https://youtu.be/EJ6_DEhC8v4?t=7602>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=7602")
...
One way to get at a column in a `data.frame` is to use the `$` operator.
...
We won't be using this very often (in fact, part of the reason to use the `tidyverse` is that we can avoid using this), but I just want you to be aware of it because it's used in the below section.

## Importing data from other software with `haven` (Optional)

Link to video: <https://youtu.be/EJ6_DEhC8v4?t=8102>

```{r}
vembedr::embed_url("https://youtu.be/EJ6_DEhC8v4?t=8102")
```
...
These all read in the data as tibbles. If the data is in SAS export (.xpt) format, we can use the `read_xpt()` function. We can also export data into these formats using the `write_*` versions of these functions such as `write_sas()`.
