...
Now we can define a function called `load_files()` that takes these file locations (or paths) as an argument for this function.
...
`map()` usually returns a list or data structure with the same number as its input; The goal of `reduce` is to take a list and return a single object.
...
-   map\_lgl() - function should return a logical value

-   map\_dbl() - function should return a double (decimal) value
