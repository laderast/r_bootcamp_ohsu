...
This is not meant to be a course in statistics. We want to show you some basic techniques, but you will need to dig further.
...
Before we do any statistical tests on our data, we should first visualize it. Since our ultimate goal is to examine the differences between bodyfat measurement methods, let's create boxplots that illustrate this difference, if any. 
...
### NULL HYPOTHESIS: There is no difference in fat mass measurements between the two methods.
...
```{r message=FALSE}
cor(body_comp$dxa, body_comp$st)

bodycomp <- body_comp %>% mutate(gender=factor(gender))

ggpairs(bodycomp,  aes(color=gender))  
```
...
What if we included `gender` in our model?

```{r}
lm(dxa ~ br + gender, data=bodycomp) %>%
  tidy()
```
